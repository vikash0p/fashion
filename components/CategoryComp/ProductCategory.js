...
export default function ProductCategory({ product }) {
    ...
    return (
        <div className='flex flex-col md:flex-row w-full lg:gap-5 min-h-screen container m-auto '>
                ...
                <div className='flex flex-col w-full gap-2 ' >
                    <h1 className='text-2xl font-semibold '>Category</h1>
                    {
                        CategoryDate.map((value, indx) => {
                            return (
                                <button type="button" key={indx} className={`  py-2 rounded-md text-left ps-4 hover:bg-secondary hover:text-black ${selected === indx ? "bg-primary" : "bg-secondary "}`} onClick={() => UpdateFilterValue(value, indx)} >{value} </button>
                            )
                        })
                    }
                </div>
                <div className='flex flex-col w-full gap-2 mt-5 ' >
                    <h1 className='text-2xl font-semibold '>Price</h1>
                    {
                        prices.map((value, indx) => {
                            return (
                                <button type="button" key={indx} className={` ${sortOrder === value ? " bg-primary" : "bg-secondary"}  py-2 rounded-md text-left ps-4 hover:bg-violet-400 hover:text-black `} onClick={() => handleSort(value)} >{value} </button>
                            )
                        })
                    }
                </div>
            ...
            <div className="basis-[100%] md:basis-[85%]  px-3">
